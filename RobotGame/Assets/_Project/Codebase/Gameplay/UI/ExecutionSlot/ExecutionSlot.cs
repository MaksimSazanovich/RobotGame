...
namespace Unity_one_love.RobotGame
{
    public class ExecutionSlot : MonoBehaviour
    {
        ...
        private void OnActionSlotDragged(float y)
        {
            ...
            int index = executionSlotsPanel.CurrentPositionList.ToList().FindIndex(y => RectTransform.position.y <= y);
            Debug.Log(index);
            if (index == -1)
                RectTransform.SetAsFirstSibling();
            else
                RectTransform.SetSiblingIndex(executionSlotsPanel.CurrentPositionList.Length - index);
        }
